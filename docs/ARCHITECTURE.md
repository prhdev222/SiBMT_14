...
**เอกสารนี้อธิบายสิ่งที่โค้ดทำจริง ณ วันที่ 22 สิงหาคม 2569** ไม่ใช่สิ่งที่ตั้งใจจะทำ
...
sequenceDiagram
    ...
    G->>G: releaseLock() ก่อนเริ่มส่งแจ้งเตือน
    G->>M: ส่งใบยืนยันนัด + referral ID
    G->>L: แจ้งกลุ่ม fellow — วันนัด เพศ อายุ โรค
...
fellow ได้รับแจ้งทาง LINE ในขั้นตอนนี้ด้วย — ส่งหลังปล่อยล็อกแล้ว
การส่งอีเมลและ LINE ใช้เวลาระดับวินาที ถ้าทำตอนยังถือล็อกอยู่ คนที่กดจองคิวถัดไปต้องรอไปด้วย
...
| จองคิวกลุ่ม 1 สำเร็จ | LINE | กลุ่ม fellow | วันนัด เพศ อายุ โรค (ไม่มีชื่อ ไม่มี HN) |
...
### ช่องว่างที่ 1 — fellow ไม่รู้ว่ามีคนจองคิวมาพบ ✅ ปิดแล้ว 11 ส.ค. 2569

เดิม `notifyFellow_` เขียนเสร็จแล้วทั้งข้อความแต่ไม่มีที่ไหนเรียกใช้เลย
`bookTransplantSlot_` เขียนคิวลงชีตแล้วส่งอีเมลให้แพทย์ต้นทางแล้วจบ fellow จึงต้องเปิดชีตดูเอง

แก้แล้วโดยเปลี่ยนชื่อเป็น `notifyFellowOfBooking_` รับ object จากการจองโดยตรง
แทนการอ่านชื่อคอลัมน์ที่เส้นทางการจองไม่ได้เขียน (เดิมอ่าน `appointment_note` ซึ่งไม่มีจริง
ข้อความจึงจะเต็มไปด้วยขีด `-`) และเรียกจาก `bookTransplantSlot_` **หลังปล่อยล็อกแล้ว**

**ข้อจำกัดที่ยังเหลือ:** `LINE_TARGET_FELLOW` เป็น group ID — fellow ทุกคนในกลุ่มเห็นข้อความ
ไม่ใช่เฉพาะคนที่ถูกจอง ข้อความจึงขึ้นชื่อ fellow ไว้บรรทัดแรกให้รู้ว่าเป็นคิวของใคร
การส่งถึงตัวบุคคลติดปัญหาเดียวกับช่องว่างที่ 2
...
### ปิดไปแล้ว 11–14 ส.ค. 2569

เดิมหัวข้อนี้ระบุว่ายังไม่ได้ deploy และยังไม่ได้จำกัดอัตราการล็อกอิน ทั้งสองข้อไม่จริงแล้ว

- **deploy ขึ้น Cloudflare Worker แล้ว** ที่ `sibmt-refer.uradev222.workers.dev`
  ผ่าน `@opennextjs/cloudflare` + `wrangler.jsonc`
- **จำกัดอัตราการล็อกอินแล้ว แต่ทำที่ Worker ไม่ใช่ WAF** — กฎ Rate limiting ของ WAF
  ผูกกับ zone ซึ่งต้องเป็นโดเมนในบัญชีเราเอง แต่ `*.workers.dev` เป็นโดเมนของ Cloudflare
  จึงไม่มีเมนู WAF ให้ตั้ง binding `LOGIN_RATE_LIMIT` ใน `wrangler.jsonc` ทำแทน
  (5 ครั้งต่อ 60 วินาที เรียกก่อนตรวจรหัสผ่าน) ถ้าวันหลังผูกโดเมนของภาควิชาแล้วจะย้ายไป WAF ก็ได้
