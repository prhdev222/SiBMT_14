 ...
 *
 * HSOS คือแหล่งตารางเวรตั้งต้น — sync **เฉพาะตอนสั่ง** (เมนู SiBMT ในชีต
 * หรือปุ่มบนหน้าเวร Chief ของเว็บ) ไม่มี trigger อัตโนมัติโดยเจตนา:
 * การแลกเวรแก้ในระบบ refer ได้เอง (หน้าเวร Chief / แก้ชีตตรง ๆ) และต้องไม่
 * ถูก sync กลางคืนเขียนทับเงียบ ๆ (มติผู้ใช้ 5 ก.ย. 2569) — การกด sync คือ
 * การประกาศว่า "เอาตาม HSOS ทั้งชุด" ปุ่มจึงเตือนก่อนเสมอว่าจะทับของที่แก้มือ
 *
 * เหตุที่เก็บลงชีตแทน query สด: HSOS ล่มระบบ refer ไม่ล้มตาม และโค้ดที่อ่าน
 * ตารางเวร (Notify.gs, OnFormSubmit.gs) ไม่ต้องรู้จัก HSOS เลย
 *
