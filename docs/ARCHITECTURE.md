...
### บรรเทาแล้วอย่างไร (22 ส.ค. 2569)

เลือกทางที่**ไม่เก็บ LINE ID ของใครเลย** แทนการทำ LIFF — ยอมให้แพทย์ "ถามเอง"
แทนที่ระบบจะ "ยิงไปหา" เพราะเคส refer รอกันเป็นวันอยู่แล้ว ความต่างจึงไม่มาก
แต่ขอบเขตข้อมูลส่วนบุคคลที่ระบบถือครองไม่กว้างขึ้นเลย

1. **ช่องอีเมลเปลี่ยนเป็นบังคับ** — เดิมเป็น "อีเมล (ถ้ามี)" เว้นว่างแล้วแพทย์
   ไม่ได้รับแม้แต่รหัสอ้างอิง เคสนั้นก็เงียบหายจนต้องโทรตาม
2. **พิมพ์รหัสอ้างอิงหาบอทในแชทตัวต่อตัว แล้วบอทตอบสถานะให้**
   (`apps-script/LineWebhook.gs` ดู [LINE_TEMPLATES.md](LINE_TEMPLATES.md) §3.4)
   ทำได้เพราะการ **ตอบกลับ** ใช้ `replyToken` ที่มากับข้อความ ต่างจากการ **push**
   ที่ต้องรู้ `userId` ล่วงหน้า

**ที่ยังทำไม่ได้:** ระบบยังยิงแจ้งเตือนหาแพทย์ต้นทางเองไม่ได้ เขาต้องเปิดมาถาม
หรือรออีเมล

**ถ้าวันหนึ่งอยากได้ push จริง ๆ:** ไม่ต้องทำ LIFF — `event.source.userId`
มากับ webhook ที่มีอยู่แล้ว แค่เก็บผูกกับ referral ID ตอนที่แพทย์พิมพ์รหัสเข้ามา
แต่ต้องยอมรับว่านั่นคือการเริ่มเก็บข้อมูลส่วนบุคคลของแพทย์ ซึ่งต้องไปแก้
ประกาศความเป็นส่วนตัวและ retention ตามด้วย

> **หมายเหตุ:** "LINE ID" ที่แพทย์รู้ (ไอดีไว้แอดเพื่อน) กับ `userId` ที่ push API
> ต้องใช้ เป็นคนละอย่างและแปลงกันไม่ได้ ตรวจกับเอกสาร LINE แล้ว —
> userId เป็นค่าที่แพลตฟอร์มสร้าง ต่างกันตาม provider เจ้าตัวเปิดดูเองไม่ได้
> และไม่มี API ไหนรับ LINE ID แล้วคืน userId ให้ **ต่อให้ใส่ช่อง "LINE ID"
> ในฟอร์มก็ push ไม่ได้อยู่ดี**
