 ...
 */
/**
 * LINE login สำเร็จแต่ LINE นี้ยังไม่เคยผูกเบอร์ — ผูกต่อบนเว็บได้เลย (11 ก.ย. 2569)
 * เดิมส่งไปพิมพ์ "ผูกบัญชี" ใน LINE ซึ่งต้องสลับแอปแล้วกลับมาใหม่ คนส่วนใหญ่หลุดตรงนั้น
 * ตอนนี้ถามเบอร์ต่อทันที ยืนยันรหัสอีเมลผ่าน = ผูก LINE นี้ให้อัตโนมัติ
 */
export function lineUnlinkedMessage(): BotMessage {
  return {
    from: "bot",
    text:
      "เข้าด้วย LINE สำเร็จ แต่ LINE นี้ยังไม่เคยผูกกับเบอร์โทรครับ\n" +
      "พิมพ์เบอร์โทรที่ใช้ตอนส่งเรื่อง (ช่อง \"เบอร์โทรกลับ\" ในฟอร์ม) " +
      "ระบบจะส่งรหัสไปอีเมลที่ลงทะเบียนไว้ พอยืนยันแล้วจะผูก LINE นี้ให้เลย " +
      "— ครั้งหน้ากด \"เข้าด้วย LINE\" ก็เห็นคำตอบทันที",
    chips: [MENU_CHIP],
  };
}

/** bubble หลัง verifyCodeAction สำเร็จและผูก LINE ให้แล้ว */
export const CODE_VERIFIED_LINKED_TEXT =
  "ยืนยันตัวตนสำเร็จ ✅ และผูก LINE นี้กับเบอร์ของท่านแล้ว ครั้งหน้ากด \"เข้าด้วย LINE\" ได้เลย";
