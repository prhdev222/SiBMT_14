...
function buildStatusReply_(referralId, found) {
  ...
  // คำถามทั่วไปไปที่แอดมินทาง LINE — ผู้อ่านอยู่ในแชทกับบอทอยู่แล้ว กดได้เลย
  message += 'สอบถามเพิ่มเติม กดปุ่ม "ติดต่อเจ้าหน้าที่" ได้เลย';
...
function buildWhoAmIReply_(sourceType, id) {
  ...
  if (sourceType === 'group' || sourceType === 'room') {
    ...
    if (!staffLine) {
      staffLine = isStaff
        ? '\n\n✅ ห้องนี้เป็นกลุ่มเจ้าหน้าที่ — พิมพ์ "เมนู" เพื่อดูคำสั่งทั้งหมด'
