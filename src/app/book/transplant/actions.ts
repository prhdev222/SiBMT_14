...
export async function bookAction(
  ...
  const missing = [
    ...
    // บังคับกรอก — ไม่ใช่แค่ความสะดวก
    //
    // อีเมลคือทางเดียวที่แพทย์ต้นทางได้รับ "ลิงก์จัดการนัด" ที่มี token
    // ถ้าเว้นว่าง จะเลื่อนหรือยกเลิกนัดเองไม่ได้เลย ต้องโทรให้เจ้าหน้าที่กดให้
    // ซึ่งเป็นเส้นทางที่ทั้งระบบนี้ตั้งใจเลิกใช้ และไม่ได้ใบยืนยันนัดด้วย
    //
    // กลุ่ม 2 และ 3 บังคับไปแล้วตั้งแต่ 22 ส.ค. ด้วยเหตุผลเดียวกัน
    !input.referrerEmail && "อีเมล",
