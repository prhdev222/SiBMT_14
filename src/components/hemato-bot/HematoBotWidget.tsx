...
function BotChoices({ onChoice }: { onChoice: (message: Message, label?: string) => void }) {
  ...
  return <div className="mt-2 flex flex-wrap gap-2"><Choice label="📅 คำถามเรื่องนัด" onClick={() => onChoice({ from: "bot", text: "กลุ่มที่ 1 เลือกวันนัดจากคิวว่างได้ทันทีค่ะ\nถ้าต้องการเลื่อน ยกเลิก หรือเปิดใบนัด ให้ใช้หน้าจัดการนัด", links: [{ label: "รายละเอียดกลุ่ม 1", href: "/refer/transplant" }, { label: "จองคิว", href: "/book/transplant" }, { label: "จัดการนัด", href: "/booking" }] }, "📅 คำถามเรื่องนัด")} /><Choice label="🧬 Transplant candidate" onClick={() => onChoice({ from: "bot", text: candidateText, links: [{ label: "ดูรายละเอียดกลุ่ม 1", href: "/refer/transplant" }] }, "🧬 Transplant candidate")} /><Choice label="🌐 กลุ่มที่ 4" onClick={() => onChoice({ from: "bot", text: "กลุ่มที่ 4 ให้ผู้ป่วยทำนัดผ่าน LINE Siriraj นัดหมายของโรงพยาบาลโดยตรงค่ะ", links: [{ label: "ดูขั้นตอนกลุ่ม 4", href: "/refer/general" }] }, "🌐 กลุ่มที่ 4")} /></div>;
}
