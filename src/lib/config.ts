...
/**
 * เครดิตระบบ — แสดงที่ footer ทุกหน้า และหน้า /about
 *
 * อาจารย์เป็นผู้ริเริ่มและกำกับทิศทาง (11 ก.ย. 2569) ชื่อ/คำนำหน้าตามที่หน่วยงานยืนยัน
 * แก้ที่นี่ที่เดียวแล้ว deploy — ขึ้นทุกหน้าพร้อมกัน
 */
export const CREDITS = {
  /** อาจารย์ผู้ริเริ่มและกำกับโครงการ — เรียงตามที่หน่วยงานกำหนด */
  directors: ["อ.ฉัตรี", "อ.ณัฐนนท์", "อ.เพชรชมพู"],
  directorsLabelTh: "ริเริ่มและกำกับโดย",
  developer: "Uradev",
  developerLabelTh: "พัฒนาโดย",
  yearTh: "2569",
} as const;

export const CONTACT = {
  officeTh: "OPD 700 โลหิตวิทยา ตึกผู้ป่วยนอก ชั้น 7 โรงพยาบาลศิริราช",
  /*
   * เบอร์และเวลาทำการของ OPD 700 (อัปเดต 31 ส.ค. 2569 ตามข้อมูลจากหน่วยงาน)
   *
   * ⚠️ เวลานี้คือ "เวลาเปิดของ OPD" สำหรับแสดงผลเท่านั้น — คนละตัวกับ
   * BUSINESS_HOURS (08:00–16:00) ซึ่งเป็นเกณฑ์นับ SLA และต้องตรงกับ
...
export const HOSPITAL_APPOINTMENT = {
  ...
  /**
   * รูป QR code เพิ่มเพื่อน LINE "Siriraj นัดหมาย"
   *
   * วางไฟล์ไว้ที่ `public/` แล้วใส่เส้นทางที่ขึ้นต้นด้วย `/` เช่น
   * `/siriraj-appointment-qr.png` — ปล่อย null ไว้หน้าเว็บจะไม่แสดงกล่อง QR
   *
   * ทำไมต้องมีรูป ไม่ใช่แค่ลิงก์: แพทย์ต้นทางเปิดหน้านี้บนจอคอมพิวเตอร์ที่ห้องตรวจ
   ...
   *
   * `link` เป็นทางลัดตรงไปยังหน้าเดียวกับที่เมนูใน LINE พาไป
   * มีไว้เพื่อให้ผู้ป่วยที่ใช้ LINE ไม่ได้ยังทำขั้นตอนนั้นได้
   * และให้แพทย์ต้นทางเปิดจากจอในห้องตรวจให้ผู้ป่วยดูได้เลย
   */
  stepsTh: [
    { text: 'สแกน QR code เพิ่มเพื่อน LINE "Siriraj นัดหมาย"' },
    {
      text: 'กดเมนู "เวชระเบียน" เพื่อทำบัตรโรงพยาบาลศิริราชออนไลน์ (เฉพาะผู้ที่ยังไม่มี HN)',
      link: {
        url: "https://si-eservice2.mahidol.ac.th/medrecord/index.php",
        labelTh: "เปิดหน้าลงทะเบียนเวชระเบียนโดยตรง",
      },
    },
    { text: 'กดเมนู "นัดหมาย" แล้วกรอกข้อมูลผู้ป่วย' },
    { text: 'แนบรูปถ่าย "ใบส่งตัว" (ถ้าแนบไม่ได้ ส่งทางแชท LINE ได้)' },
    {
      text:
        "รอเจ้าหน้าที่ติดต่อกลับเพื่อรับใบนัดหมาย ภายใน 3 วันทำการ " +
        "โดยผู้ป่วยจะได้รับวันนัดพบ hematologist ที่ OPD อายุรศาสตร์ ภายใน 1-2 สัปดาห์",
    },
    { text: 'ดูนัดหมายและชำระค่าบริการครั้งถัดไปได้ที่เมนู "Siriraj Connect"' },
    { text: "มาตรวจตามวันเวลาที่ได้รับจาก LINE" },
  ] as {
    text: string;
    link?: { url: string; labelTh: string };
  }[],
  waitingDaysTh: "3 วันทำการ",
  /**
   * ⚠️ กลุ่มนี้ไม่ให้เบอร์ OPD 700
   *
   * ผู้ป่วยกลุ่ม 4 เข้าระบบนัดหมายของโรงพยาบาลไปที่ OPD อายุรศาสตร์โดยตรง
   * ไม่ได้ผ่าน OPD 700 ซึ่งเป็นคลินิกโลหิตวิทยา — การให้เบอร์ที่นั่นจะทำให้
   * ผู้ป่วยโทรไปหาหน่วยงานที่ไม่มีข้อมูลนัดของตัวเองและตอบอะไรไม่ได้
   *
   * ช่องทางถามคือแชท LINE ของระบบนัดหมายเอง ซึ่งเป็นที่เดียวที่เห็นคิวของผู้ป่วย
   * ถ้าวันหนึ่งมีเบอร์ของ OPD อายุรศาสตร์ ให้ใส่ตรงนี้แล้วเปิดใช้ในหน้ากลุ่ม 4
   */
  contactPhone: null as string | null,

  /** เบอร์หน่วยเวชระเบียนผู้ป่วยนอก — คนละเรื่องกับการนัดหมาย ใช้ตอนทำบัตรไม่ผ่าน */
  medicalRecordPhoneTh: "02-419-7323, 02-419-9990",
};
