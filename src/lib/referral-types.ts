...
export const REFERRAL_TYPE_META: Record<ReferralType, ReferralTypeMeta> = {
  TRANSPLANT_APPOINTMENT: {
    ...
    purposeTh:
      "ต้องการนัดผู้ป่วยมาพบ fellow transplant ที่ OPD 700 โลหิตวิทยา ตึกผู้ป่วยนอก ชั้น 7 โรงพยาบาลศิริราช เพื่อประเมินการปลูกถ่ายไขกระดูก/สเต็มเซลล์",
...
export interface Referral {
  ...
  /**
   * true เมื่ออีเมลที่แพทย์ต้นทางกรอกน่าจะใช้ไม่ได้ — สองอีเมลในฟอร์มไม่ตรงกัน
   * (`email_mismatch`) หรือมีการออกลิงก์ยืนยันแล้ว (`email_verify_token` ไม่ว่าง)
   * แต่ยังไม่มีใครกดยืนยันเกิน 1 วันทำการ (`email_verified_at` ว่างและ
   * `elapsed_business_hours` >= 8) — ดู Task 5
   *
   * ⚠️ ต้องเช็ก email_verify_token ก่อนเสมอ — เคสที่ส่งก่อนมีฟีเจอร์นี้ไม่เคยมี
   * ลิงก์ยืนยันให้กด ไม่ใช่แค่ยังไม่ว่างกด ถ้าไม่เช็กจะขึ้นธงเตือนเท็จ
   * (false positive) ทั้งหมดตั้งแต่วันเปิดใช้
   *
   * เคสกลุ่ม 2/3 พึ่งอีเมลนี้ในการส่งคำแนะนำกลับ ถ้าธงนี้ขึ้นแปลว่าคำตอบ
   * อาจไปไม่ถึงแพทย์ต้นทาง ต้องโทรยืนยันเบอร์แทน
   */
