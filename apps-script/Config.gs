...
/**
 * กลุ่มที่ 1 เก็บไว้กี่วันหลังเลยวันนัด ก่อนถอดชื่อ (มติอาจารย์ 30 ส.ค. 2569)
 *
 * ⚠️ นับจากวันนัด ไม่ใช่ closed_at เหมือนกลุ่มอื่น
 *
 * เคสกลุ่มที่ 1 ที่ผู้ป่วยมาตามนัดจะไม่มี closed_at เลย เพราะสถานะหยุดอยู่ที่
 * 'Appointment Confirmed' ตั้งแต่วันจอง ถ้าใช้กฎเดียวกับกลุ่มอื่นแถวนั้นจะ
 * ไม่มีวันครบกำหนดและค้างพร้อมชื่อ–เบอร์–อีเมลแพทย์ต้นทางตลอดกาล
 *
 * ⚠️ เหลือช่วงผ่อนผันไว้ ไม่ได้ลบทันทีที่เลยวันนัด
 *
 * เพราะรอบถอดชื่อทำงานเดือนละครั้ง ถ้าไม่มีช่วงผ่อนผัน เคสที่นัดวันสิ้นเดือน
 * จะหายไปในวันรุ่งขึ้น ส่วนเคสที่นัดต้นเดือนอยู่ได้เกือบสามสิบวัน — อายุข้อมูล
 * จะขึ้นกับว่าบังเอิญนัดวันไหนของเดือน ซึ่งอธิบายให้ใครฟังไม่ได้
 * สามสิบวันทำให้ทุกเคสได้เวลาเท่ากันอย่างน้อยหนึ่งเดือนสำหรับทบทวนคลินิกที่ผ่านมา
 */
const APPOINTMENT_RETENTION_DAYS = 30;
